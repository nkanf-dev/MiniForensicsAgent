...
class LlamaCPPHTTPClient:
    ...
    def generate(self, prompt: str, config: Any) -> dict[str, Any]:
        ...
        resp.raise_for_status()
        data = resp.json()
        content = data.get("choices", [{}])[0].get("text", "") or data.get("content", "")
        self.last_usage = {
            "prompt_tokens": data.get("timings", {}).get("prompt_n", 0),
            "completion_tokens": data.get("timings", {}).get("predicted_n", 0),
        }
        return {"text": content}
    ...
    def generate_stream(self, prompt: str, config: Any) -> Generator[str, None, None]:
        ...
        for line in resp.iter_lines():
            ...
            if line.startswith("data: "):
                ...
                try:
                    obj = json.loads(raw)
                    content = obj.get("choices", [{}])[0].get("text", "") or obj.get("content", "")
                    if content:
                        yield content
